...
class Settings:
    ...
    bank_app_links: dict[str, tuple[str, str]]
    payment_qr_image: Path | None
...
def load_settings() -> Settings:
    ...
    default_app_links = {
        "MBANK": (
            "https://play.google.com/store/apps/details?id=com.maanavan.mb_kyrgyzstan",
            "https://apps.apple.com/kg/app/id922922121",
        ),
        "O!Деньги": (
            "https://play.google.com/store/apps/details?id=kg.o.nurtelecom",
            "https://apps.apple.com/kg/app/id1257075492",
        ),
        "BAKAI": (
            "https://play.google.com/store/apps/details?id=kg.bta.mobilebank2",
            "https://apps.apple.com/kg/app/id1294108123",
        ),
        "MegaPay": (
            "https://play.google.com/store/apps/details?id=com.kp.megapay.kg",
            "https://apps.apple.com/kg/app/id1317275502",
        ),
    }
    bank_app_links = {
        key: (
            os.getenv(f"PAYMENT_{key}_ANDROID_URL", "").strip() or android_url,
            os.getenv(f"PAYMENT_{key}_IOS_URL", "").strip() or ios_url,
        )
        for key, (android_url, ios_url) in default_app_links.items()
    }
    qr_raw = os.getenv("PAYMENT_QR_IMAGE", "assets/payment_qr.png").strip()
    qr_path = Path(qr_raw)
    if not qr_path.is_absolute():
        qr_path = Path(__file__).resolve().parent / qr_path
    ...
    return Settings(
        ...
        bank_urls=banks,
        bank_app_links=bank_app_links,
        payment_qr_image=qr_path if qr_path.is_file() else None,
    )
...
def payment_link_keyboard(
    ...
    rows: list[list[InlineKeyboardButton]] = []
    if android_url.startswith("https://"):
        rows.append([InlineKeyboardButton(text=f"📲 Открыть {bank} — Android", url=android_url)])
    if ios_url.startswith("https://"):
        rows.append([InlineKeyboardButton(text=f"🍎 Открыть {bank} — iPhone", url=ios_url)])
    rows.append([InlineKeyboardButton(text=CANCEL, callback_data="cancel")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
    ...
    if settings.payment_qr_image:
        try:
            await message.answer_photo(
                FSInputFile(settings.payment_qr_image),
                caption=(
                    "🏦 <b>Единый QR-код для оплаты</b>\n\n"
                    "Отсканируйте QR в приложении выбранного банка. "
...
async def bank_selected(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    android_url, ios_url = settings.bank_app_links.get(bank, ("", ""))
    ...
    text += "\n\nВыберите приложение вашего банка ниже."
    await callback.message.answer(
        text,
        reply_markup=payment_link_keyboard(bank, android_url, ios_url),
    )
